...
\begin{figure}
\centering
\includegraphics[width=\textwidth,height=2.08333in]{./img/moth_body_3.png}
\caption{Convex hull}
\end{figure}
...
\begin{figure}
\centering
\includegraphics[width=\textwidth,height=2.08333in]{./img/moth_body_4.png}
...
\hypertarget{running-the-program-for-execution}{%
\subsection{Running the program for
execution}\label{running-the-program-for-execution}}
...
For each individual moth, a folder should be created with the same name
as the name of the insect. This name will later be inserted into the
results table. Inside the folder there must be files whose names can be
anything, but at the end of the file name there must be postfixes:
\textbf{body}, \textbf{left\_wing} or \textbf{right\_wing}.

To run the program for execution, you need to open the file in the
\textbf{Rstudio} program and execute the command \textbf{Code} →
\textbf{Source}

\includegraphics{./img/RStudio.png} After execution, a file called
\textbf{data.csv} will appear in the folder.

\begin{figure}
\centering
\includegraphics[width=6.25in,height=\textheight]{./img/data.png}
\caption{The results of the calculations}
\end{figure}

To run the program on \emph{Palmetto} cluster for high performance, you
need to execute the the following commands.
...
\CSLLeftMargin{2. }
\CSLRightInline{Smith DS (1965)
\href{https://doi.org/10.1242/jeb.204.23.4125}{Flight muscles of
insects}. \emph{Scientific American} 212: 76--88.}
...
\CSLLeftMargin{4. }
\CSLRightInline{Canny J (1986)
\href{http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.420.3300\&rep=rep1\&type=pdf}{A
computational approach to edge detection}. \emph{IEEE Transactions on
Pattern Analysis and Machine Intelligence} 8: 679--698.}

\leavevmode\vadjust pre{\hypertarget{ref-Alpha-shape}{}}%
\CSLLeftMargin{5. }
\CSLRightInline{Knorr EM, Ng RT, Shilvock DL (1997)
\href{https://doi.org/10.1007/3-540-63238-7_23}{Finding boundary shape
matching relationships in spatial data}, In: Scholl M, Voisard A (Eds.),
\emph{Advances in spatial databases}, Berlin, Heidelberg, Springer
Berlin Heidelberg, 29--46.}

\leavevmode\vadjust pre{\hypertarget{ref-Shoelace-formula}{}}%
\CSLLeftMargin{6. }
\CSLRightInline{Allgower PH Eugene L.; Schmidt (1986)
\href{https://doi.org/10.2307\%2F2008221}{Computing volumes of
polyhedra}. \emph{Mathematics of Computation} 46: 171--174.}
