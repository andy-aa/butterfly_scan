...
![Convex hull](./img/moth_body_3.png){height="200px"}
...
![Axis of rotation](./img/moth_body_4.png){height="200px"}
...
![Superposition of polygons](./img/Areas_3_4_5_6_Hemaris_diffinis_left_wing.png){height="300px"}
...
## Running the program for execution
Image files should be located as shown in the following figure.

![Folder tree](./img/files.png)

For each individual moth, a folder should be created with the same name as the name of the insect.
This name will later be inserted into the results table. Inside the folder there must be files whose names can be anything, but at the end of the file name there must be postfixes: **body**, **left_wing** or **right_wing**. 

To run the program for execution, you need to open the file in the **Rstudio** program and execute the command **Code** → **Source**

![Folder tree](./img/RStudio.png)
After execution, a file called **data.csv** will appear in the folder.

![The results of the calculations](./img/data.png){width="600px"}


To run the program on *Palmetto* cluster for high performance, you need to execute the the following commands.

Copy files to palmetto:
```
$ git clone https://github.com/andy-aa/butterfly_scan.git
```

Make a reservation computing node:
```
$ qsub -I -l select=1:ncpus=4:mem=10gb:interconnect=1g,walltime=2:00:00
```
Load the R language module:
```
